...
const ViewCampaignsContainer = ({ role }) => {
  ...
  const fetchAdminCampaigns = async () => {
    try {
      const response = await axios.post(
        "https://sand-pbmk.onrender.com/api/v1/admin/fetchAllCampaigns",
        { clientId }
      );
  ...
  const fetchMisCampaigns = async () => {
    try {
      ...
      const response = await axios.post(
        "https://sand-pbmk.onrender.com/api/v1/mis/fetchMisCampaigns",
        { misId: id }
      );
  ...
  const handleDeleteClient = async () => {
    try {
      const response = await axios.delete(
        "https://sand-pbmk.onrender.com/api/v1/admin/deleteClient",
        {
          data: { clientId },
        }
      );
